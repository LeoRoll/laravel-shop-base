...
class EmailVerificationController extends Controller
{
	public function verify(Request $request)
	{
		...
		// 从缓存中读取数据，我们把从 url 中获取的 'token' 与缓存中的值在做对比
		...
		// 将指定的 key 从缓存中删除，由于已经完成了掩验证，这个缓存就没有必要继续保留。
		Cache::forget('email_verification_'.$email);
		//最关键的，要把对应用户的 'email_verified' 字段改为 'true'。
		$user->update(['email_verified' => true]);

		// 最后告知用户邮箱验证成功。
		return view('pages.success', ['msg' => '恭喜您，邮箱验证成功，欢迎来到本商城！']);
	}
	...
	public function send(Request $request)
	{
		...
		if ($user->email_verified) {
			throw new InvalidRequestException('尊敬的用户您好，您的邮箱已经验证通过了，无需再做验证。');
		}
